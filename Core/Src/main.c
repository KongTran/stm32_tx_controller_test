...
struct Signal {
  uint16_t adc_buffer[4];  
	uint16_t servo_aux_x;
	uint16_t servo_aux_y;
};
struct Signal adc;

void adc_dma_init(){
	HAL_ADC_Start_DMA(&hadc1,(uint32_t *)adc.adc_buffer,4);
}
...
void esc_bldc_write(uint16_t pwm_val[4]){
	uint16_t pwm[4];
	for(int i=0;i<4;i++){
		pwm[i]=(pwm_val[i]*1000)/4096+1000;
	}
